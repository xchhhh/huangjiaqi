...
function respawnSticker(sticker, index, total) {
  ...
  sticker.speed = rand(0.35, 0.8);
  sticker.drift = rand(0.8, 2.0);
  sticker.rot = rand(-25, 25);
  sticker.spin = rand(-0.04, 0.04);
...
function makeSticker(index) {
  ...
  el.addEventListener('pointerenter', () => { sticker.hoverScale = 1.12; });
  el.addEventListener('pointerleave', () => { sticker.hoverScale = 1; });
...
function animate(now) {
  ...
  for (let i = 0; i < stickers.length; i++) {
    ...
    // 直接设置 transform 确保移动端也能正常重绘
    const finalScale = sticker.scale * sticker.hoverScale;
    sticker.el.style.transform = `translate3d(${sticker.x.toFixed(1)}px, ${sticker.y.toFixed(1)}px, 0) rotate(${sticker.rot.toFixed(1)}deg) scale(${finalScale.toFixed(2)})`;
  }
